...
default_instrumentation_package="splunk-otel-systemd-auto-instrumentation"
old_instrumentation_package="splunk-otel-auto-instrumentation"
default_instrumentation_version="latest"
default_deployment_environment=""
instrumentation_properties_path="/usr/lib/splunk-instrumentation/splunk-otel-javaagent.properties"
instrumentation_systemd_config_path="/etc/systemd/system.conf.d/00-splunk-otel-javaagent.conf"
...
install_yum_package() {
  ...
  opt=""
  if [ "$package_name" = "$old_instrumentation_package" ]; then
    opt="--setopt=obsoletes=0"
  fi

  if command -v yum >/dev/null 2>&1; then
    yum $opt install -y ${package_name}${version}
  elif command -v dnf >/dev/null 2>&1; then
    dnf $opt install -y ${package_name}${version}
  else
...
ensure_not_installed() {
  ...
  if [ "$with_instrumentation" = "true" ]; then
    ...
    if [ -f "$instrumentation_jar_path" ]; then
      echo "$instrumentation_jar_path already exists which implies that the Java Auto Instrumentation agent has already been installed." >&2
      echo "Please uninstall the Java Auto Instrumentation agent and re-run this script" >&2
      exit 1
    fi
  fi
}
...
update_instrumentation_config() {
  local deployment_environment="$1"
  local instrumentation_package="$2"
  local config_path="$instrumentation_properties_path"

  if [ "$instrumentation_package" = "$old_instrumentation_package" ]; then
    config_path="$instrumentation_config_path"
  fi

  if [ -f "$config_path" ]; then
    ts="$(date '+%Y%m%d-%H%M%S')"
    echo "Backing up $config_path as ${config_path}.bak.${ts}"
  ...
  if [ "$config_path" = "$instrumentation_config_path" ]; then
    ...
    echo "# Auto-generated by the splunk-otel-collector.sh installer script on $(date)" > "$config_path"
    if [ -n "$deployment_environment" ]; then
      echo "otel.resource.attributes=deployment.environment=${deployment_environment}" >> "$config_path"
    fi
    if [ -n "$service_name" ]; then
      echo "otel.service.name=${service_name}" >> "$config_path"
    fi
    echo "splunk.profiler.enabled=${enable_profiler}" >> "$config_path"
    echo "splunk.profiler.memory.enabled=${enable_profiler_memory}" >> "$config_path"
    echo "splunk.metrics.enabled=${enable_metrics}" >> "$config_path"
  fi
}
...
install() {
  ...
  local instrumentation_package="$7"
  ...
  case "$distro" in
    ubuntu|debian)
      ...
      if [ -n "$instrumentation_version" ]; then
        install_apt_package "$instrumentation_package" "$instrumentation_version"
      fi
      ;;
    amzn|centos|ol|rhel)
      ...
      if [ -n "$instrumentation_version" ]; then
        install_yum_package "$instrumentation_package" "$instrumentation_version"
      fi
      ;;
    sles|opensuse*)
      if [ "$skip_collector_repo" = "false" ]; then
...
uninstall() {
  for agent in otelcol td-agent $instrumentation_so_path $instrumentation_jar_path; do
    if command -v $agent >/dev/null 2>&1; then
      pkg="$agent"
      if [ "$agent" = "otelcol" ]; then
        pkg="splunk-otel-collector"
      elif [ "$agent" = "$instrumentation_so_path" ]; then
        pkg="$old_instrumentation_package"
      elif [ "$agent" = "$instrumentation_jar_path" ]; then
        pkg="$default_instrumentation_package"
      fi
      case "$distro" in
        ubuntu|debian)
          if dpkg -s $pkg >/dev/null 2>&1; then
            if [ "$pkg" != "$old_instrumentation_package" ] && [ "$pkg" != "$default_instrumentation_package" ]; then
...
Options:
  ...
  --with[out]-instrumentation           Whether to install and configure the Splunk OpenTelemetry Auto Instrumentation
                                        package.
  ...
  --[no-]generate-service-name          *DEPRECATED*: Specify '--no-generate-service-name' to prevent the preloader from
                                        ...
                                        Only applicable if both the '--with-instrumentation' and
                                        '--instrumentation-package $old_instrumentation_package' options are also
                                        specified.
  ...
  --instrumentation-package <name>      The Splunk OpenTelemetry Auto Instrumentation package name to install
                                        ('$default_instrumentation_package' or '$old_instrumentation_package').
                                        *WARNING*: '$old_instrumentation_package' is *DEPRECATED* and no longer
                                        supported.
                                        Only applicable if the '--with-instrumentation' option is also specified.
                                        (default: $default_instrumentation_package)
  --instrumentation-version <version>   The Splunk OpenTelemetry Auto Instrumentation package version to install.
                                        Only applicable if the '--with-instrumentation' option is also specified.
                                        (default: $default_instrumentation_version)
  -- <access_token>                     Use '-- <access_token>' if access_token starts with '-'.
...
parse_args_and_install() {
  ...
  local instrumentation_package="$default_instrumentation_package"
  ...
  while [ -n "${1-}" ]; do
    case $1 in
      ...
      --instrumentation-package)
        case $2 in
          $default_instrumentation_package|$old_instrumentation_package)
            instrumentation_package="$2"
            ;;
          *)
            echo "Unsupported instrumentation package '$2'" >&2
            exit 1
            ;;
        esac
        shift 1
  ...
  if [ "$with_instrumentation" = "true" ]; then
    echo "Splunk OpenTelemetry Auto Instrumentation ($instrumentation_package):"
    echo "  Version: $instrumentation_version"
    if [ -n "$deployment_environment" ]; then
    ...
    echo "  AlwaysOn CPU Profiling enabled: ${enable_profiler}"
    echo "  AlwaysOn Memory Profiling enabled: ${enable_profiler_memory}"
    echo "  Metrics Collection enabled: ${enable_metrics}"
  fi
  ...
  install "$stage" "$collector_version" "$td_agent_version" "$skip_collector_repo" "$skip_fluentd_repo" "$instrumentation_version" "$instrumentation_package"

  if [ "$with_instrumentation" = "true" ]; then
    update_instrumentation_config "$deployment_environment" "$instrumentation_package"
  fi
...
Reboot the system or restart the Java application(s) for auto instrumentation to start.

EOH
    else
      cat <<EOH
The Splunk OpenTelemetry Auto Instrumentation package has been installed, and the Java agent has
been enabled via the JAVA_TOOL_OPTIONS environment variable defined in the
${instrumentation_systemd_config_path} drop-in file for Java applications running as systemd services.
${instrumentation_properties_path} has been configured based on the installer arguments and defaults.

Reboot the system or restart the Java systemd service(s) for auto instrumentation to start.
